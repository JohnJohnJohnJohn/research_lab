...
**v0.1 COMPLETE — Discord bridge done. CLI + Discord both working.**

Next: **v0.2** (Bloomberg MCP, full HK + China analyst activation, FactorRegime live).
...
5. ~~**Discord bridge**~~ — **done** (v0.1.6): discord.py Gateway, threaded memos, slash commands, thread feedback. See `DISCORD_SETUP.md`. Slack replaced (HK accessibility).
...
| v0.1.5 Slack | ~~Slack Socket Mode bridge~~ **Superseded v0.1.6** — Slack not reliably accessible in HK; replaced by Discord. | See v0.1.6 Discord bridge. |
| v0.1.6 Discord | **Discord bridge via discord.py Gateway.** `handle_discord_message()` posts parent memo + thread sections; slash commands `/research`, `/rerun`, `/rerun-all`, `/lock`, `/macro`; thread replies for feedback. No emoji reactions. | SPEC §7–§9 adapted for Discord. Setup: `DISCORD_SETUP.md`. |
| v0.1.6 Discord | `_last_task` / `_last_ticker` in-memory only; resets on bot restart. | Cross-session persistence v0.2. |
| v0.1.6 Discord | `/rerun` and `/rerun-all` run **full pipeline** in v0.1; partial step re-runs deferred to v0.2. | Task spec v0.1 scope. |
...
| 2026-05-28 | Discord bridge agent | discord.py Gateway replaces Slack; threaded memos, 5 slash commands, DISCORD_SETUP.md. v0.1 complete. |
